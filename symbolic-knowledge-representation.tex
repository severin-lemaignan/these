...
\begin{tikzpicture}[taxonomy]
    \node [taxon] {\bf B. Representation}
            ...
            child {node [taxon] {{\bf B.4}. Self-knowledge}}
...
(and possibly past and future) state of the robot. Note that these description
...
with applications for high-level multi-robot cooperation. Recent works in that
direction include~\cite{Kunze2011}.
...
(storing numerical value, let alone matrices, in OWL is cumbersome and not efficient due to the serialisation to XML).

\subsubsection{Metrics and quality criteria}

Qualitative and quantitative metrics give an insight on the size and complexity
of ontologies.
