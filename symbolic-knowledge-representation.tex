...
Indeed, pervasive knowledge could safely be considered as the proeminent
...
architectures for robots. Their main strenghts are underline, in order to
...
information to uncontextualized symbolic facts.
...
theoritical framework that study the links between cognition, embodiment and
actions).
...
maintainance of a link between the a symbol (the syntactic form of knowledge
...
mediating minds is classically refered as the \emph{semantic triangle}, and has
...
the situation ``a familly meal'', the fact ``the human is sitting at the
...
The typology has been built from three main sources: a review of the lexisting
...
learning/remembering/introspection. They also separatly identify four
...
question: what can be possibly represented. When it explicitely exists, the
...
its different aspects in both natural (human) and artifical systems.
...
persepectives computed by the robot.
...
information and knowledge, as discussed at the begining of the chapter).
...
controler and are mostly unvisible to the knowledge representation system.
...
\begin{quote}
The convential wisdom of software engineering is that one should
develop independent modules that have mini- mal interaction. In contrast, a
...
Finally, because service robots are design to act in rich, dynamic
...
in cognitive psychology. Several ``classical'' tests (like False-Belief
...
However, the performance evaluation of current tools is still mostly system-
and scenario-specific (``did my system do the job for my task?''), and amongst
the surveyed KRS, only few systems proposes an evaluation of their performances
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\columnwidth]{stateofart/gsm.pdf}
