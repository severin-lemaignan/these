...
The ORO server is written in the Java 6 programming language. This choice of
...
Communication with dedicated robotic middlewares like ROS or YARP is provided
...
Modules that implement the \jinterface{IServiceProvider} interface expose
\emph{services} (\jclass{IService}), which are actual methods exposed by the
server. Java method to be exposed are simply marked with the
\jinterface{@RPCMethod} Java annotation, and automatically added to the server
API. Extending the server API is thus simply a matter of annotating any Java
method with \jinterface{@RPCMethod}. Listing~\ref{code|oro-add} shows one
...
in use in the robotic community, {\sc Yarp}~\cite{Metta2006} and
ROS~\cite{Quigley2009}.

These wrappers use the C++ or Python bindings previously presented to expose
the functionalities of ORO server as a stand-alone node.
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\columnwidth]{implementation/oroview.jpg}
...
concepts} (instances of \concept{ActiveConcept}. Each time an individual is
asserted to be an \concept{ActiveConcept}, the server triggers back {\tt
